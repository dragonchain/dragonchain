...
def select_transaction(location: str, block_id: str, txn_id: str) -> dict:
    ...
    try:
        obj = s3.select_object_content(
            ...
            Expression=f"select s.txn, s.stripped_payload, s.stripped_payload_by_block from s3object s where s.txn_id = '{txn_id}' limit 1",  # nosec (this s3 select query is safe)
            ExpressionType="SQL",
        ...
        if loaded_txn.get("stripped_payload"):
            ...
            if does_object_exist(location, payload_key):
                ...
                if loaded_block.get(txn_id):
                    loaded_txn["txn"]["payload"] = json.loads(loaded_txn[txn_id].decode("utf-8"))
                else:
                    loaded_txn["txn"]["payload"] = json.dumps({})
