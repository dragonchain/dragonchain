...
elif [ "$arch" = "armv7" ] || [ "$arch" = "armv7l" ] || [ "$arch" = "armv7hl" ]; then
    TAG="linux-armv7-latest"
    docker build . -f cicd/Dockerfile.dependencies.armv7 -t "dragonchain/dragonchain_core_dependencies:$TAG" --no-cache --pull
else
...
if docker manifest create dragonchain/dragonchain_core_dependencies:latest dragonchain/dragonchain_core_dependencies:linux-amd64-latest dragonchain/dragonchain_core_dependencies:linux-arm64-latest dragonchain/dragonchain_core_dependencies:linux-armv7-latest; then
